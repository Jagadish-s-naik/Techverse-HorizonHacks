...
import { generateDriverBullets } from '../utils/ai.js';
...
export function computeForecast(prices) {
...
export async function processTarget(cropInput, mandiInput) {
    ...
    if (todayPrice) {
        ...
        await query(`INSERT INTO mandi_prices (crop, mandi, date, price) 
       VALUES ($1, $2, CURRENT_DATE, $3)
       ON CONFLICT (crop, mandi, date) DO UPDATE SET price = EXCLUDED.price`, [crop, mandi, todayPrice]);
    }
    // 2. Get last 30 days of prices
    let priceData = await query('SELECT price FROM mandi_prices WHERE crop = $1 AND mandi = $2 ORDER BY date DESC LIMIT 30', [crop, mandi]);
    ...
    const result = await query(`INSERT INTO forecasts (crop, mandi, forecast_date, price_low, price_high, trend, confidence, drivers)
     ...
     RETURNING *`, [crop, mandi, forecast.band.low, forecast.band.high, forecast.trend, forecast.confidence, JSON.stringify(drivers)]);
...
export async function recordActuals() {
    ...
    const pending = await query(`SELECT f.id, f.crop, f.mandi, f.forecast_date 
     FROM forecasts f
    ...
    for (const forecast of pending.rows) {
        ...
        const actual = await query('SELECT price FROM mandi_prices WHERE crop = $1 AND mandi = $2 AND date = $3', [forecast.crop, forecast.mandi, forecast.forecast_date]);
        if (actual.rows.length > 0) {
            // 3. Record the actual price
            await query('INSERT INTO forecast_actuals (forecast_id, actual_price) VALUES ($1, $2)', [forecast.id, actual.rows[0].price]);
        }
    }
}
