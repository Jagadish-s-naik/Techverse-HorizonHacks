...
export async function runForecastPipeline() {
    ...
    for (const target of targets.rows) {
        ...
        const priceData = await query('SELECT price FROM mandi_prices WHERE crop = $1 AND mandi = $2 ORDER BY date DESC LIMIT 30', [crop, mandi]);
        if (priceData.rows.length < 14)
            continue;
        ...
        // 5. Save to DB
        await query(`INSERT INTO forecasts (crop, mandi, forecast_date, price_low, price_high, trend, confidence, drivers)
       ...
       drivers = EXCLUDED.drivers`, [crop, mandi, forecast.band.low, forecast.band.high, forecast.trend, forecast.confidence, JSON.stringify(drivers)]);
    }
}
